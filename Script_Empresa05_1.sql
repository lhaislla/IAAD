...
begin;
create schema empresa_lhaislla; -- inclua seu nome
-- O comando 'create database' é semelhante ao comando 'create schema'
use empresa_lhaislla;
...
insert into FUNCIONARIO values
 ...
 insert into DEPARTAMENTO values
	...
	('Recursos_Humanos',2,NULL,NULL),
    ('Tecnologia_Informação',3,NULL,NULL);		
...
-- Letra A:
SELECT Dnome FROM DEPARTAMENTO WHERE Dnumero NOT IN (SELECT Dnr FROM FUNCIONARIO);
-- Letra B:
SELECT Dnome FROM DEPARTAMENTO, FUNCIONARIO WHERE Dnumero = Dnr AND Cpf IS NULL;
-- Letra C:
SELECT Dnome FROM DEPARTAMENTO D WHERE NOT EXISTS (SELECT * FROM FUNCIONARIO F WHERE F.Dnr = D.Dnumero);
-- Letra D: 
SELECT Dnome FROM DEPARTAMENTO LEFT JOIN FUNCIONARIO ON Dnumero = Dnr WHERE Dnr IS NULL;
-- Letra E:
SELECT Dnome FROM DEPARTAMENTO, FUNCIONARIO WHERE Dnumero != Dnr;
