...
-- Chaves Estrangeiras
alter table PEDIDO ADD FOREIGN KEY(PCodCli) REFERENCES CLIENTE (codCli)ON ;
alter table ITEM_PEDIDO ADD FOREIGN KEY (IcodProd) REFERENCES PRODUTO(CodProd)
ON DELETE RESTRICT 
ON UPDATE RESTRICT;
alter table ITEM_PEDIDO ADD FOREIGN KEY (InumPedido) REFERENCES PEDIDO(NumPedido)DELETE CASCADE;
commit;
